function [ estimate, successrate ] = validateRobustClassifier( mysvm, FlyStruct, arenalist )
...
for i = arenalist
    for m = 1:size(FlyStruct(i).Validation,2)
        testfeatures = [ testfeatures; FlyStruct(i).Validation{m}.AverageTSIRZ' FlyStruct(i).Validation{m}.LBRZ'];
        t_class = [ t_class 2-FlyStruct(i).Validation{m}.trueState ];
    end
end
for i = 1:2
    estimate(i,:) = svmclassify(mysvm{i}, testfeatures,'Showplot',true)';
end
